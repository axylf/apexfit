  ...
  const fetchMessage = async (chatHistory) => {
    try {
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [{ role: "system", content: "You are a health, wellness, and nutrition assistant for ApexFit. You can only provide information based on these categories." }, ...chatHistory],
          max_tokens: 150,
        }),
      });
